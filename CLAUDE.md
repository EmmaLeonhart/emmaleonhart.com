...
- `github_bio/readme.md` — **first-class: the canonical source of Emma's GitHub *profile* README.** It is NOT just a doc — it syncs to the `EmmaLeonhart/EmmaLeonhart` profile repo (the bio shown on her GitHub profile) via `.github/workflows/sync-profile-readme.yml`, which writes it into that repo's README between the `<!-- BIO:START -->` / `<!-- BIO:END -->` markers (daily, on push to this file, and on manual dispatch). **Edit the profile bio HERE — never directly on the profile repo; the sync overwrites the marked region.** The workflow needs the `PROFILE_README_TOKEN` repo secret (fine-grained PAT, contents:write on `EmmaLeonhart/EmmaLeonhart`) and Actions enabled on the profile repo. Treat this file with the same care as `resume.md` — it is a public-facing source of truth.
