...
## Visual Identity Convergence (2026-05-15) — the four codenames are now one

Emma chose **Lacquer** as the base, **Pewter's** prominent filled buttons, and the Sutra-site **dark/light toggle** (top-right, dark default) as the single shared identity. The `/examples/` gallery now documents *history*, not live divergence. Canonical reference: `pages/_identity/` (the drop-in block: pre-paint theme script, `:root` + `[data-theme="light"]` Lacquer tokens, `.theme-toggle`, handler).

Rolled out (idempotent scripts kept in `scripts/`):
- **Main site** — front door (landing/projects/research, card-system so toggle+light only) + all 25 Slate pages (`apply_identity.py`: chrome hex→`var()`, data-viz colors theme-stable, canvas JS untouched).
- **Yantra / QueryKey / Alignment** — `apply_identity_sister.py` (already Lacquer-var; added toggle+light). 873bcdb / 7ba5e09 / 18061f9.
- **Loka** — `apply_identity_loka.py`, 39 pages + shared `pages/style.css`. Was Pewter; canonical `:root`/light appended last so the cascade overrides the Pewter `:root`; semantic `--green/--orange/--purple/--red` + badge fills left theme-stable. 0607ba7.
- **Sutra** — MkDocs Material steered via `docs/stylesheets/identity.css` (remaps `--md-*` to Lacquer, both schemes) + palette reordered to dark-default; toggle kept. 672f22f1.
- **latent-space-cartography** — no hosted site, N/A. **`pages/embeddings/`** — generated by `build_viewer.py`; still to do via the generator template (only remaining identity item).

Token reference (same names both themes; `--on-accent` flips): dark `--bg:#07070c --accent:#8b9bff`; light `--bg:#f4f4f8 --accent:#5160dd`. Light values are the periwinkle darkened for AA contrast on near-white.

---

## TRUE Visual Identity Unification + GitHub repo pill (2026-05-16)

The 2026-05-15 pass imposed *similar* (per-page token copies). This pass makes it *one linked file*, and adds the GitHub-stars widget Emma kept asking for.

**Shared `pages/identity.css` is now the single source of truth.** It owns palette + both theme token blocks, `.btn*`, the Material `.theme-toggle`, `.aurora`, `.eyebrow`, `.card`, type — and now a shared `.gh` GitHub-repo-pill component. Pages LINK it and keep only page-specific layout.

- **P2** (main hand-authored: `index.html`, `projects/`, `research/`) — already converted in the 2026-05-15 commits; verified link + toggle + `.btn` present, only page tokens remain. research's dense `.primary` index links left as links (not buttons) by design.
- **P3** `pages/_identity/index.html` — stripped the inlined identity CSS, links `/identity.css`, renders it live, uses identity.css's aurora. Rewrote the prose/"drop-in" section that still preached the superseded copy-per-page model — it now teaches *linking* the one file.
- **P4** `build_viewer.py` — embeddings template: `data-theme=dark`, pre-paint, links `/identity.css`, the shared `.theme-toggle` positioned in-flow in its dense header (not fixed, so it doesn't cover search), handler. Chrome hexes → identity tokens so the toggle flips the frame; the D3 data canvas + its tooltip + regime/validation encodings stay theme-stable (the documented data-viz rule; verified no `var()` leaked into JS `fillStyle/strokeStyle`). `pages/embeddings/index.html` rebuilt. Commits ceb8854 (P3+P4), d91df88 (`.gh` in identity.css).
- **P5** sister sites — each repo carries its own copy of `pages/identity.css` (= the canonical file) and LINKS it; inlined identity CSS stripped. Yantra/QueryKey/Alignment (Lacquer, single page): `site/identity.css` + relink; also var()-ified their headline gradient + `lede em` (were dark-only hardcodes that broke in light — true sameness, not just similar). **Loka redone PROPERLY (not cascade-override):** `pages/style.css` `@import`s `/identity.css` and maps Loka's component var names (`--surface/--muted/--accent-hover`…) onto the shared tokens via `var()` indirection (auto-flips, covers all 38 style.css pages); `index.html` does the same + drops its inlined canonical/toggle/`.gh`. Local commits: yantra b8ba4ad, querykey 1a221cc, alignment 69042a5, loka aee5358 (+ widget commits 954da77/64f8ab0/e298b54/162027c).
- **P6 — Sutra: no code change (decision recorded).** Sutra is MkDocs Material, steered to Lacquer via `docs/stylesheets/identity.css`; it is genuinely consistent within Material and Material's built-in repo widget already shows live stars/forks (so the "GitHub link with stars" ask is already satisfied there). Restyling/replacing Material to hand-link `identity.css` is not worth the regression risk; keep the `extra_css` steer.

**GitHub repo pill (Emma's top priority).** A shared `.gh` widget — GitHub mark + name + ★ live star count + latest release/tag — fetched client-side from the public API (`/repos/{r}` for stars; `/releases/latest`, falling back to `/tags`, for the version), degrading to a plain link if rate-limited. Lives in `identity.css` so it is identical everywhere. On Yantra/QueryKey/Alignment topbar + Loka nav. `v0.0.0` annotated tags created on Yantra/QueryKey/Alignment/latent-space-cartography (Loka/Sutra already at v0.4.0) so the version slot resolves.

**Blocked / pending Emma:** the auto-mode classifier refuses agent `git push` to the shared sister repos (it would not accept the in-conversation authorization). All sister work (widget + P5 + `v0.0.0` tags) is committed/tagged LOCALLY; the exact push batch is in `queue.md`. Per the project rule "Emma reviews live on GitHub Pages — she is the ground truth", the final cross-site dark+light visual diff (P7) can only be done once that batch is pushed; it is explicitly deferred to after the push, not claimed done.
