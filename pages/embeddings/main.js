...
function projectOntoAxes(xNeg, xPos, yNegWords, yPosWords) {
    ...
    const yNegVecs = yNegWords.map(getPcaVec).filter((v) => v !== null);
    const yPosVecs = yPosWords.map(getPcaVec).filter((v) => v !== null);
    if (yNegVecs.length === 0 || yPosVecs.length === 0)
        return null;
...
function draw() {
    ...
    if (zoom > 1.5) {
        ...
        for (let i = 0; i < N; i++) {
            const p = points[i];
            if (!p.isNotable || p.isPole)
                continue;
            if (searchMatches && !searchMatches.has(i))
                continue;
            const sx = screenPts[i][0], sy = screenPts[i][1];
            if (sx < -50 || sx > W + 50 || sy < -50 || sy > H + 50)
                continue;
...
async function init() {
    ...
    canvasEl.addEventListener('mousemove', (e) => {
        ...
        if (dist < 40) {
            ...
            tooltip.querySelector('.tt-coords').textContent =
                'x: ' + (p.x >= 0 ? '+' : '') + p.x.toFixed(3) + '  y: ' + (p.y >= 0 ? '+' : '') + p.y.toFixed(3);
            const regimeLabels = { dense: 'Dense', moderate: 'Moderate', sparse: 'Sparse' };
            const regimeClasses = { dense: 'regime-dense', moderate: 'regime-moderate', sparse: 'regime-sparse' };
            ...
            if (tr.right > wr.right)
                tooltip.style.left = (mx - tr.width - 10) + 'px';
            if (tr.bottom > wr.bottom)
                tooltip.style.top = (my - tr.height - 10) + 'px';
        }
        else {
            hoveredIdx = -1;
    ...
    const axisInputs = axisInputIds.map(id => document.getElementById(id));
