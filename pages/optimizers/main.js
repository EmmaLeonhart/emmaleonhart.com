...
function resetOptimizers() {
    ...
    for (const o of optimizers)
        enabledState.set(o.key, o.enabled);
    optimizers = OPT_DEFS.map(d => {
        const o = makeOptimizer(d);
        if (enabledState.has(d.key))
            o.enabled = enabledState.get(d.key);
        return o;
    });
}
function stepOptimizer(o) {
    if (!o.enabled)
        return;
