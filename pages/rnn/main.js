...
let wXH = []; // [N_HIDDEN][N_INPUT]  input→hidden
let wHH = []; // [N_HIDDEN][N_HIDDEN] hidden→hidden (recurrence!)
let bH = []; // [N_HIDDEN]
let wHY = []; // [N_OUTPUT][N_HIDDEN] hidden→output
let bY = []; // [N_OUTPUT]
// Forward pass state per time step
let hiddenStates = []; // [nSteps][N_HIDDEN]
let outputStates = []; // [nSteps][N_OUTPUT]
// Canvas
let canvas;
let ctx;
let W = 0, H = 0;
...
function drawCurvedArrow(x1, y1, x2, y2, color, width, curveUp = true) {
    const mx = (x1 + x2) / 2;
...
function drawNeuron(x, y, val, fillColor, label) {
    // Glow
    if (Math.abs(val) > 0.1) {
        ctx.beginPath();
    ...
    for (let t = 0; t < nSteps; t++) {
        ...
        for (let h = 0; h < N_HIDDEN; h++) {
            for (let i = 0; i < N_INPUT; i++) {
                ...
                drawArrowLine(inputXs[i], layerY[0] + NEURON_R, hiddenXs[h], layerY[1] - NEURON_R, weightColor(wXH[h][i], alpha), 1);
            }
        }
        // Draw hidden neurons
        for (let h = 0; h < N_HIDDEN; h++) {
        ...
        for (let h = 0; h < N_HIDDEN; h++) {
            ...
            drawArrowLine(hiddenXs[h], layerY[1] + NEURON_R, cx, oy - NEURON_R, weightColor(wHY[0][h], alpha), 1);
        }
        // Output neuron
        ...
        if (t < nSteps - 1) {
            ...
            drawCurvedArrow(fromX + NEURON_R, layerY[1], toX - NEURON_R, layerY[1], COLORS.recurrentArrow, 2, true);
            // Label
            ctx.font = '8px "Segoe UI", system-ui, sans-serif';
...
function init() {
    ...
    document.querySelectorAll('.ctrl-btn[data-steps]').forEach(btn => {
        btn.addEventListener('click', () => {
            nSteps = parseInt(btn.dataset.steps || '3');
