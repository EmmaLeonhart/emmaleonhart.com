...
// Shows how predicate-based exit conditions terminate property path
// traversal early instead of exhaustively exploring the entire graph
export {};
...
function addNode(id: string, label: string, x: number, y: number, year: number, diedInOffice: boolean): void {
...
// --- State ---
interface State {
  phase: 'idle' | 'no-until' | 'with-until' | 'done';
  running: boolean;
  mode: 'none' | 'no-until' | 'with-until';
...
function buildNoUntilSteps(): (() => void)[] {
  const steps: (() => void)[] = [];
...
function buildWithUntilSteps(): (() => void)[] {
  const steps: (() => void)[] = [];
  ...
  for (let i = 0; i < chain.length; i++) {
    ...
    if (node.diedInOffice) break; // Don't generate steps past exit
  }
  return steps;
}
