// Graph Database (SPARQL/RDF) Visualization
// Shows how SPARQL traverses RDF triples to answer multi-hop queries
export {};
...
function addPerson(id: string, label: string, x: number, y: number): void {
...
interface SearchState {
  ...
  running: boolean;
  // Which triples are currently highlighted
  matchedTriples: Set<number>;
  activeNodes: Set<string>;
  traversedEdges: Set<string>; // "s->o" format
  bindings: { person: string; father: string; grandfather: string; greatgrandfather: string }[];
...
function draw(): void {
  ...
  for (const t of triples) {
    if (t.predicate !== ':hasFather') continue;
    ...
    drawArrow(from, to,
      isTraversed ? COLORS.fatherEdge : COLORS.edge,
      isTraversed ? 2.5 : 1,
      ':hasFather'
    );
  }
