// Vector Database HNSW Visualization
// Shows how HNSW traversal finds nearest neighbors through hierarchical graph layers
export {};
...
interface HNSWNode {
  ...
  embedding: number[]; // simplified 2D "embedding" for visualization
  json: { fatherId?: string; [k: string]: unknown };
...
const nodes: Map<string, HNSWNode> = new Map();

function addNode(id: string, name: string, x: number, y: number, layer: number, embedding: number[], json: HNSWNode['json']): void {
  nodes.set(id, { id, name, pos: { x, y }, layer, embedding, json, neighbors: new Map() });
}

function addEdge(a: string, b: string, layer: number): void {
  const na = nodes.get(a)!;
  const nb = nodes.get(b)!;
  if (!na.neighbors.has(layer)) na.neighbors.set(layer, []);
  if (!nb.neighbors.has(layer)) nb.neighbors.set(layer, []);
...
interface SearchState {
  ...
  currentNode: string;
  visited: Set<string>;
  path: { nodeId: string; layer: number; dist: number }[];
  results: string[];
  phase: 'idle' | 'traversing' | 'done';
  stepQueue: (() => void)[];
  stepIndex: number;
}
...
function draw(): void {
  ...
  for (const node of nodes.values()) {
    for (const [layer, neighbors] of node.neighbors) {
      if (visLayer >= 0 && layer > visLayer) continue;
      for (const nId of neighbors) {
        if (nId < node.id) continue; // avoid double-draw
        const nb = nodes.get(nId)!;
        const p1 = toScreen(node.pos.x, node.pos.y);
        const p2 = toScreen(nb.pos.x, nb.pos.y);

        const isOnPath = state.path.some((s, i) =>
          i > 0 && s.layer === layer &&
...
function buildSearchSteps(): (() => void)[] {
  const steps: (() => void)[] = [];
