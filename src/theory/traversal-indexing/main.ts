...
interface GraphNode {
  ...
  label: string;
  pos: Vec2;
  pageRank: number; // 0.0 - 1.0 normalized
  traversalCount: number; // how often traversed
  isEntry: boolean;
}
...
function addNode(id: string, label: string, x: number, y: number, pr: number, tc: number): void {
  nodes.set(id, { id, label, pos: { x, y }, pageRank: pr, traversalCount: tc, isEntry: false });
}

function addEdge(from: string, to: string, pred: string): void {
...
// --- State ---
interface State {
  phase: 'idle' | 'pagerank' | 'counters' | 'entry-select' | 'traversal' | 'done';
  running: boolean;
  // Visualization
...
function draw(): void {
  ...
  for (const node of nodes.values()) {
    ...
    if (state.showPageRank) {
      ...
      if (node.pageRank > 0.7) color = COLORS.hotNode;
      else if (node.pageRank > 0.3) color = COLORS.warmNode;
      else color = COLORS.coldNode;
    }

    if (state.showCounters) {
      if (node.traversalCount > 400) color = COLORS.hotNode;
      else if (node.traversalCount > 150) color = COLORS.warmNode;
      else color = COLORS.coldNode;
    }
