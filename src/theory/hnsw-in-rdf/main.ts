...
interface RdfTriple {
  ...
  p: string;
  o: string;
  type: 'data' | 'vector' | 'hnsw';
}

const triples: RdfTriple[] = [];

function addNode(id: string, label: string, x: number, y: number, layer: number, hasEmb: boolean): void {
  nodes.set(id, { id, label, pos: { x, y }, layer, hasEmbedding: hasEmb });
}

function addTriple(s: string, p: string, o: string, type: RdfTriple['type']): void {
...
// --- State ---
interface State {
  phase: 'idle' | 'data-triples' | 'vector-triples' | 'hnsw-triples' | 'unified-query' | 'done';
  running: boolean;
  visibleTypes: Set<RdfTriple['type']>;
  activeTriples: Set<number>;
...
function buildSteps(): (() => void)[] {
  ...
  steps.push(() => {
    ...
    for (let i = 0; i < triples.length; i++) {
      if (triples[i].type === 'data') state.activeTriples.add(i);
    }
    state.steps.push({
  ...
  steps.push(() => {
    ...
    for (let i = 0; i < triples.length; i++) {
      if (triples[i].type === 'vector') state.activeTriples.add(i);
    }
    state.steps.push({
  ...
  steps.push(() => {
    ...
    state.activeTriples.clear();
    for (let i = 0; i < triples.length; i++) {
      if (triples[i].type === 'hnsw') state.activeTriples.add(i);
    }
    state.steps.push({
  ...
  steps.push(() => {
    ...
    for (let i = 0; i < triples.length; i++) {
      const t = triples[i];
      if (t.s === 'paper1' && t.p === 'sutra:hnswNeighbor' && t.o === 'paper2') state.activeTriples.add(i);
      if (t.s === 'paper6' && t.p === 'sutra:hnswNeighbor' && t.o === 'paper1') state.activeTriples.add(i);
      if (t.s === 'paper2' && t.p === ':discusses' && t.o === 'topic1') state.activeTriples.add(i);
      if (t.s === 'paper1' && t.p === ':discusses' && t.o === 'topic1') state.activeTriples.add(i);
    }
