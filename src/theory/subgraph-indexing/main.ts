// Subgraph Indexing for SIMD in Graph Databases
// Shows how pseudo-tables flatten repeated subgraph patterns into columnar indexes
// that enable SIMD-accelerated scanning instead of pointer-chasing
export {};
...
// --- Graph data: Country → Capital → Mayor pattern ---
interface GraphNode {
  id: string;
  label: string;
  pos: Vec2;
  type: 'country' | 'city' | 'person' | 'date';
}
...
function addNode(id: string, label: string, x: number, y: number, type: GraphNode['type']): void {
  graphNodes.set(id, { id, label, pos: { x, y }, type });
}

function addTriple(s: string, p: string, o: string): void {
...
// --- Columnar pseudo-table representation ---
interface PseudoTableRow {
  country: string;
  capital: string;
  mayor: string;
  birthDate: string;
...
const pseudoTable: PseudoTableRow[] = [
...
// --- State ---
interface State {
  phase: 'idle' | 'graph-scan' | 'discovery' | 'columnar' | 'simd-scan' | 'done';
  running: boolean;
  // Graph scan highlighting
  activeEdges: Set<string>;
  activeNodes: Set<string>;
  // Columnar view
  showTable: boolean;
  scanColumn: number; // which column is being scanned (-1 = none)
  scanRow: number; // which row the SIMD block is at
  matchedRows: Set<number>;
  // Steps
  steps: { description: string; detail: string; active: boolean; done: boolean }[];
  stepQueue: (() => void)[];
  stepIndex: number;
}
...
function toScreen(nx: number, ny: number): Vec2 {
  const mx = 50, my = 40;
  return {
    x: mx + nx * (W - 2 * mx),
    y: my + ny * (H - 2 * my),
...
function drawArrow(from: Vec2, to: Vec2, color: string, lineWidth: number, label?: string): void {
...
function drawGraph(): void {
  ...
  for (const t of triples) {
    ...
    drawArrow(from, to,
      isActive ? COLORS.predEdge : COLORS.edge,
      isActive ? 2 : 1,
      t.predicate
    );
  }
