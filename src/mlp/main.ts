// Multilayer Perceptron — Interactive Forward Pass Visualizer
// 2 inputs → 3 hidden (with activation) → 1 output
export {};

type ActivationFn = 'relu' | 'sigmoid' | 'tanh';

let activationFn: ActivationFn = 'relu';
...
let wIH: number[][] = [];  // [N_HID][N_IN]
let bH: number[] = [];     // [N_HID]
let wHO: number[][] = [];  // [N_OUT][N_HID]
let bO: number[] = [];     // [N_OUT]

// Current input values
let inputs: number[] = [0.5, 0.8];

// Forward pass state
let hiddenRaw: number[] = [];
let hiddenAct: number[] = [];
let outputRaw: number[] = [];
...
let posIn: { x: number; y: number }[] = [];
let posHid: { x: number; y: number }[] = [];
let posOut: { x: number; y: number }[] = [];
...
function activationFormula(x: string): string {
  switch (activationFn) {
...
function signalAlpha(val: number): number {
  return 0.15 + Math.min(1, Math.abs(val)) * 0.85;
}

function drawConnection(from: { x: number; y: number }, to: { x: number; y: number }, weight: number, signal: number): void {
...
function drawNeuron(pos: { x: number; y: number }, val: number, fillColor: string, label: string): void {
...
function drawLayerLabel(x: number, label: string): void {
...
function draw(): void {
  ...
  // Layer labels
  if (posIn.length > 0) drawLayerLabel(posIn[0].x, 'Input');
  if (posHid.length > 0) drawLayerLabel(posHid[0].x, 'Hidden');
  if (posOut.length > 0) drawLayerLabel(posOut[0].x, 'Output');
...
let weightSliders: WeightSlider[] = [];
...
function buildWeightSliders(): void {
...
function refreshWeightSliders(): void {
