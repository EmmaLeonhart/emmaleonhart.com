...
// --- Unified node: has both vector embedding AND graph relationships ---
interface UnifiedNode {
  id: string;
  name: string;
  pos: Vec2;
  embedding: number[]; // 2D simplified embedding
  fatherId?: string;
  // HNSW similarity neighbors (vector edges)
  vectorNeighbors: string[];
}

const nodes: Map<string, UnifiedNode> = new Map();

function addNode(id: string, name: string, x: number, y: number, emb: number[], fatherId?: string): void {
  nodes.set(id, { id, name, pos: { x, y }, embedding: emb, fatherId, vectorNeighbors: [] });
}

function addVectorEdge(a: string, b: string): void {
