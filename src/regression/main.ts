// Linear vs Logistic Regression — Interactive Visualizer
// Click to add points, toggle between models, watch the fit update in real time
export {};

type Mode = 'linear' | 'logistic';

interface Point {
  x: number;   // data x coordinate (arbitrary range)
  y: number;   // class label: 0 or 1
}
...
// ============================================================
function yMin(): number { return mode === 'logistic' ? Y_MIN : Y_MIN_LIN; }
function yMax(): number { return mode === 'logistic' ? Y_MAX : Y_MAX_LIN; }

function dataToCanvasX(x: number): number {
  return ((x - X_MIN) / (X_MAX - X_MIN)) * W;
}

function dataToCanvasY(y: number): number {
  return H - ((y - yMin()) / (yMax() - yMin())) * H;
}

function canvasToDataX(cx: number): number {
  return X_MIN + (cx / W) * (X_MAX - X_MIN);
}

function canvasToDataY(cy: number): number {
...
// UI UPDATES
// ============================================================
function updateParamDisplay(): void {
