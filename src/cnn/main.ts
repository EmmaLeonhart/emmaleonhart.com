// Convolutional Neural Network — Interactive Visualizer
// Shows how a kernel slides over a 2D input, producing a feature map
export {};
...
function setEdgeDetectKernel(): void {
  kernel = [
    [-1, -1, -1],
    [-1,  8, -1],
    [-1, -1, -1],
...
function setSharpenKernel(): void {
  kernel = [
    [ 0, -1,  0],
    [-1,  5, -1],
    [ 0, -1,  0],
...
function valColor(v: number): string {
  if (v > 0.001) return COLORS.posWeight;
  if (v < -0.001) return COLORS.negWeight;
  return COLORS.zeroWeight;
}

function intensityColor(v: number, baseColor: string): string {
...
function buildMathPanel(): void {
  ...
  let sum = 0;
  const products: { input: number; kernel: number; product: number }[] = [];
...
function buildKernelSliders(): void {
...
function moveKernel(dr: number, dc: number): void {
...
function init(): void {
  ...
  document.addEventListener('keydown', (e) => {
    if (animating) return;
    switch (e.key) {
      case 'ArrowUp': moveKernel(-1, 0); e.preventDefault(); break;
      case 'ArrowDown': moveKernel(1, 0); e.preventDefault(); break;
      case 'ArrowLeft': moveKernel(0, -1); e.preventDefault(); break;
      case 'ArrowRight': moveKernel(0, 1); e.preventDefault(); break;
    }
  });
