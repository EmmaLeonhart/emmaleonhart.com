// Regularization Techniques — Interactive Visualizer
// Weight penalties (L1/L2/Elastic Net) + Dropout & Batch Norm
export {};
...
type RegType = 'l1' | 'l2' | 'elastic';
type VizType = 'dropout' | 'batchnorm';

let regType: RegType = 'l1';
let vizType: VizType = 'dropout';
let lambda = 2.0;
let alpha = 0.5;
let dropoutRate = 0.5;

// Canvas contexts
let c1: HTMLCanvasElement, ctx1: CanvasRenderingContext2D;
let c2: HTMLCanvasElement, ctx2: CanvasRenderingContext2D;
let W1 = 0, H1 = 0, W2 = 0, H2 = 0;

// Dropout state
let droppedNeurons: boolean[] = [];
const NET_LAYERS = [4, 6, 2]; // input, hidden, output

// Batch norm demo values
let batchValues: number[] = [];
let batchNormed: number[] = [];
let bnGamma = 1.0;
let bnBeta = 0.0;
...
// ============================================================
function init(): void {
  c1 = $('canvas1') as HTMLCanvasElement;
  c2 = $('canvas2') as HTMLCanvasElement;
...
function resize(): void {
  ...
  for (const [canvas, ctxRef] of [[c1, ctx1], [c2, ctx2]] as [HTMLCanvasElement, CanvasRenderingContext2D][]) {
...
function bindControls(): void {
  ...
  document.querySelectorAll('[data-reg]').forEach(btn => {
    btn.addEventListener('click', () => {
      regType = (btn as HTMLElement).dataset.reg as RegType;
  ...
  document.querySelectorAll('[data-viz]').forEach(btn => {
    btn.addEventListener('click', () => {
      vizType = (btn as HTMLElement).dataset.viz as VizType;
  ...
  // Lambda slider
  const lambdaSlider = $('lambda-slider') as HTMLInputElement;
  ...
  // Alpha slider
  const alphaSlider = $('alpha-slider') as HTMLInputElement;
  ...
  // Dropout slider
  const dropSlider = $('dropout-slider') as HTMLInputElement;
...
function updateAlphaVisibility(): void {
  const wrap = $('alpha-wrap');
  wrap.style.display = regType === 'elastic' ? 'flex' : 'none';
}

function updateDropoutVisibility(): void {
...
function updateInsight(): void {
  ...
  const regInsights: Record<RegType, string> = {
  ...
  const vizInsights: Record<VizType, string> = {
...
function drawCanvas1(): void {
  ...
  const toX = (w: number) => cx + w * scale;
  const toY = (w: number) => cy - w * scale; // y inverted
...
function drawLossContours(ctx: CanvasRenderingContext2D, cx: number, cy: number, scale: number): void {
...
function drawConstraintRegion(ctx: CanvasRenderingContext2D, cx: number, cy: number, scale: number): void {
  const r = lambda;
  const steps = 120;
  let color: string;
  let label: string;
...
function findConstrainedOptimum(): [number, number] {
  // Brute-force search along the constraint boundary
  let bestLoss = Infinity;
  let bestW: [number, number] = [0, 0];
  const steps = 360;
  ...
  let inside = false;
  if (regType === 'l1') inside = Math.abs(uc[0]) + Math.abs(uc[1]) <= lambda;
  else if (regType === 'l2') inside = uc[0] ** 2 + uc[1] ** 2 <= lambda;
  else {
    const l1ok = Math.abs(uc[0]) + Math.abs(uc[1]) <= lambda;
    const l2ok = uc[0] ** 2 + uc[1] ** 2 <= lambda;
    inside = alpha > 0.5 ? l1ok : l2ok; // rough check
  }
  if (inside) bestW = [uc[0], uc[1]];
...
function drawBatchNorm(): void {
  ...
  const valToY = (v: number) => topY + barH - ((v - minV) / range) * barH;
