// Traditional Hybrid Database Visualization
// Shows the clunky handoff between vector search and graph database via JSON
export {};
...
interface GraphPerson {
  id: string;
  name: string;
  pos: Vec2; // position in graph viz
  fatherId?: string;
}
...
const graphPeople: Map<string, GraphPerson> = new Map();
function addG(id: string, name: string, x: number, y: number, fatherId?: string): void {
...
interface HybridState {
  ...
  results: { person: string; greatgrandfather: string; chain: string }[];
  stepQueue: (() => void)[];
  stepIndex: number;
  operations: number;
  systemBoundaries: number;
}

let state: HybridState = createInitialState();

function createInitialState(): HybridState {
  return {
    phase: 'idle',
    running: false,
    currentPhaseIndex: -1,
...
function updatePhasesUI(): void {
  const container = document.getElementById('phases-container')!;
  container.innerHTML = '';
  for (const phase of state.phases) {
    const card = document.createElement('div');
    card.className = `phase-card ${phase.status}`;
    const colorMap: Record<string, string> = { active: '#7c8cf8', done: '#34d399', error: '#f87c7c', pending: '#666880' };
    card.innerHTML = `
