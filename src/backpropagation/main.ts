// Backpropagation & Computational Graphs — Interactive Visualizer
// Shows a computational graph with forward and backward pass
export {};

interface Node {
  id: string;
  label: string;
  op: string;      // 'input' | 'mul' | 'add' | 'sigmoid' | 'mse'
  value: number;
  grad: number;
  x: number;
  y: number;
  inputs: number[]; // indices of input nodes
}
...
function backwardPass(): void {
  // Reset all gradients
  for (const n of nodes) n.grad = 0;
...
function computePositions(): void {
  ...
  const colX = (i: number) => 80 + i * ((W - 160) / (cols.length - 1));
...
function valColor(v: number): string {
  if (v > 0.001) return COLORS.posVal;
  if (v < -0.001) return COLORS.negVal;
  return COLORS.dimText;
}

function nodeColor(n: Node): string {
  if (n.op === 'input') return COLORS.inputFill;
  if (n.op === 'square' || n.op === 'sub') return COLORS.lossFill;
  if (n.op === 'sigmoid') return COLORS.outputFill;
  return COLORS.opFill;
}

function drawArrowBetween(from: Node, to: Node, color: string, width: number, dashed: boolean, label: string, above: boolean): void {
...
function drawNode(n: Node, idx: number): void {
...
function draw(): void {
  ...
  for (let i = 0; i < nodes.length; i++) {
    for (const inputIdx of nodes[i].inputs) {
      drawArrowBetween(nodes[inputIdx], nodes[i], COLORS.forwardArrow, 1.5, false,
        nodes[inputIdx].value.toFixed(2), true);
    }
  }
  ...
  if (showBackward) {
    for (let i = 0; i < nodes.length; i++) {
      for (const inputIdx of nodes[i].inputs) {
        drawArrowBetween(nodes[i], nodes[inputIdx], COLORS.backwardArrow, 1.5, true,
          '\u2202=' + nodes[inputIdx].grad.toFixed(3), false);
      }
    }
  }
...
function init(): void {
  ...
  document.querySelectorAll('.ctrl-btn[data-label]').forEach(btn => {
    btn.addEventListener('click', () => {
      trueY = parseFloat((btn as HTMLElement).dataset.label || '1');
  ...
  $('btn-step').addEventListener('click', () => {
    ...
    // Update sliders
    ($(('slider-w')) as HTMLInputElement).value = String(inputW);
    $('slider-w-val').textContent = inputW.toFixed(2);
    ($(('slider-b')) as HTMLInputElement).value = String(inputB);
